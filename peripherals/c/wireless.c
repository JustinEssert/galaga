...
static __INLINE uint8_t wireless_reg_read(uint8_t reg)
{
  ...
  tx_cmd[0] = (reg&0x1F) | NRF24L01_CMD_R_REGISTER;
	
  wireless_CSN_low();
  spiTx(RF_SPI_BASE, tx_cmd, 2, rx_data);
  wireless_CSN_high();
...
static __INLINE void wireless_reg_write(uint8_t reg, uint8_t data)
{
  ...
  tx_cmd[0] = (reg&0x1F) | NRF24L01_CMD_W_REGISTER;
  tx_cmd[1] = data;
...
static __INLINE void wireless_set_tx_addr(uint8_t  *tx_addr)
{
  ...
  tx_cmd[0] = NRF24L01_CMD_W_REGISTER | NRF24L01_TX_ADDR_R;
  tx_cmd[1] = *(tx_addr +0 );
  tx_cmd[2] = *(tx_addr +1 );
  tx_cmd[3] = *(tx_addr +2 );
  tx_cmd[4] = *(tx_addr +3 );
  ...
  wireless_CSN_low();
  spiTx(RF_SPI_BASE, tx_cmd, 6, rx_data);
  wireless_CSN_high();
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
  ...
  tx_cmd[0] = NRF24L01_CMD_W_TX_PAYLOAD;
  tx_cmd[1] = *(tx_data +3 );
  tx_cmd[2] = *(tx_data +2 );
  tx_cmd[3] = *(tx_data +1 );
  tx_cmd[4] = *(tx_data +0 );
  ...
  wireless_CSN_low();
  spiTx(RF_SPI_BASE, tx_cmd, 5, rx_data);
  wireless_CSN_high();
}
...
static __INLINE void wireless_rx_data_payload( uint32_t *data)
{
	uint8_t tx_cmd[5];
	uint8_t rx_data[5];
	uint8_t *return_data = (uint8_t *)data;

  tx_cmd[0] = NRF24L01_CMD_R_RX_PAYLOAD;
	
  wireless_CSN_low();
  spiTx(RF_SPI_BASE, tx_cmd, 5, rx_data);
  wireless_CSN_high();
	
  return_data[0] = *(rx_data +4 );
  return_data[1] = *(rx_data +3 );
  return_data[2] = *(rx_data +2 );
  return_data[3] = *(rx_data +1 );
